...
def inss_deduction(salary: float, inss_table: Inss_Table) -> float:
    aliquot_sum: float = 0.0
    for inss_range in inss_table.values():
        if salary >= inss_range.final:
            aliquot_sum += (inss_range.final - inss_range.initial) * inss_range.aliquot
        else:
            aliquot_sum += (salary - inss_range.initial) * inss_range.aliquot
    return aliquot_sum

def income_tax_deduction(salary: float, income_tax_table: Income_Tax_Table, inss_deduction: float) -> float:
    income_tax_sum: float = 0.0
    for income_tax_range in income_tax_table.values():
        if income_tax_range.initial <= salary and salary <= income_tax_range.final:
            income_tax_sum = ((salary - inss_deduction) * income_tax_range.aliquot) - income_tax_range.monthly_deduction
            break
...
class Static_Financial_Status:
    ...
    def to_json(self) -> dict[str, ...]:
        return {
            ...
            "income_tax_percentage": self.income_tax_percentage,
            "planned_work_hours": {
                "per_day": self.planned_work_hours.per_day,
                "per_week": self.planned_work_hours.per_week,
                "per_month": self.planned_work_hours.per_month,
                "per_year": self.planned_work_hours.per_year
            },
            "planned_salary": {
                "per_month": self.planned_salary.per_month
            }
        }
...
if __name__ == "__main__":
    ...
    financial_status = Static_Financial_Status(
        ...
        income_tax_percentage=0.275,
        inss_table=inss_table,
        planned_work_hours=Static_Financial_Status.Planned_Work_Hours(
            per_day=10.0, per_week=40.0, per_month=160.0, per_year=1920
        ),
        planned_salary=Static_Financial_Status.Planned_Salary(
            per_month=10000.0
        ),
    ...
    salary = 10000
    inss_deduction_val = inss_deduction(salary, inss_table)
    print(inss_deduction_val, income_tax_deduction(salary, income_tax_table, inss_deduction_val))
